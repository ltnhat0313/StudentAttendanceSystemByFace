...
def admin_schedule_add(request):
    if request.method == 'POST':
        ...
        lecturer_id = request.POST.get('id_lecturer', '').strip()

        lecturer = None
        if lecturer_id:
            lecturer = StaffInfo.objects.filter(id_staff=lecturer_id).first()
            if lecturer is None:
                messages.warning(request, 'Mã giảng viên không tồn tại. Lớp học sẽ được lưu mà không gán giảng viên.')
...
def admin_schedule_edit(request, id_classroom):
    ...
    if request.method == 'POST':
        ...
        lecturer_id = request.POST.get('lecturer_name', '').strip()
        lecturer = None
        if lecturer_id:
            lecturer = StaffInfo.objects.filter(id_staff=lecturer_id).first()
            if lecturer is None:
                messages.warning(request, 'Mã giảng viên không tồn tại. Hệ thống sẽ bỏ gán giảng viên cho lớp học này.')
...
def admin_list_student_in_class_add(request, classroom_id):
    if request.method == 'POST':
        ...
        student = StudentInfo.objects.filter(id_student=id_student).first()
        if student is None:
            student = StudentInfo(
        ...
        try:
            student_in_class = StudentClassDetails(id_classroom_id=classroom_id, id_student=student)
            student_in_class.save()
            messages.success(request, 'Thêm sinh viên vào lớp học thành công.')
        except Exception as exc:
            messages.error(request, f'Không thể thêm sinh viên vào lớp học: {exc}')
...
def main():
    ...
    with tf.Graph().as_default():
        with tf.compat.v1.Session() as sess:
            ...
            valid_dataset = []
            for cls in dataset:
                valid_paths = [path for path in cls.image_paths if os.path.exists(path)]
                if valid_paths:
                    cls.image_paths = valid_paths
                    valid_dataset.append(cls)
                else:
                    print(f'Skipping class {cls.name}: no valid image files found')

            if not valid_dataset:
                TRAIN_STATUS = 0
                output_string = 'Không có ảnh hợp lệ nào để huấn luyện.'
...
def train(request):
    try:
        ...
        return JsonResponse({'train': train_result, 'resuil': resuil, 'error': None})
    except Exception as exc:
        import traceback
        traceback.print_exc()
        return JsonResponse({'train': 0, 'resuil': f'Lỗi khi training: {exc}', 'error': str(exc)}, status=500)
